<?php
if(!class_exists("C7ACC195B")) {
	class C7ACC195B {
    	public $key = "91DC018A2B541A15";
    	private $product_id = "3";
    	private $product_base = "xamin-wordpress";
    	private $server_host = "http://iqonic.design/license/wp-json/licensor/";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_xamin-wordpress_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=xamin-wordpress_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
